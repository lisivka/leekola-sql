...
def create_in_progress_working_hours_view(conn):
    try:
        ...
        cursor = conn.execute('SELECT issue_key, author_key, start_date, end_date FROM in_progress_working_hours')
        ...
        conn.commit()
        print("View in_progress_working_hours created and populated successfully.")
    except sqlite3.Error as e:
