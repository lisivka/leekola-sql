...
# Функція для створення таблиці для робочого часу
def create_in_progress_working_hours_table(conn):
    try:
        conn.execute('''
        CREATE TABLE IF NOT EXISTS in_progress_working_hours (
            issue_key TEXT,
            author_key TEXT,
            status TEXT,
            start_date TEXT,
        ...
        for row in rows:
            ...
            conn.execute('''
            INSERT INTO in_progress_working_hours (issue_key, author_key, status, start_date, end_date, working_seconds)
            VALUES (?, ?, ?, ?, ?, ?)
            ''', (issue_key, author_key, status, start_date, end_date, working_seconds))

        conn.commit()
        print("Table in_progress_working_hours created and populated successfully.")
    except sqlite3.Error as e:
